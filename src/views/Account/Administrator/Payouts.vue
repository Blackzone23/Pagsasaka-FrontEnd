                ...
                <table class="w-full border-collapse border border-gray-300 rounded-md min-w-[600px]">
                    <thead class="sticky top-0 bg-gray-300 z-10">
                        <tr class="text-xs sm:text-sm md:text-md">
                            ...
                            <th class="px-3 sm:px-4 py-2 text-left border-b border-gray-300">Time Slot</th>
                            <th class="px-3 sm:px-4 py-2 text-left border-b border-gray-300"> Queue Number</th>
                            <th class="px-3 sm:px-4 py-2 text-left border-b border-gray-300">Seller Name</th>
                            <th class="px-3 sm:px-4 py-2 text-left border-b border-gray-300"> Code</th>
                            <th class="px-3 sm:px-4 py-2 text-left border-b border-gray-300">Amount </th>
                            <th class="px-3 sm:px-4 py-2 text-left border-b border-gray-300">Status </th>
                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="payment in paymentList" :key="payment.id" class="border-b border-gray-200 hover:bg-gray-200 text-xs sm:text-sm">
                            ...
                            <td class="px-3 sm:px-4 py-2 text-center">{{ payment.queue_number }}</td>
                            ...
                            <td class="px-3 sm:px-4 py-2 text-end">
                                <button @click="approvePayment(payment.id)"  class="bg-[#608C54] text-white py-1 px-3 rounded-md text-xs hover:bg-green-700 transition" :disabled="payment.status !== 'Pending'">
                                    Approve
                                </button>
                            </td>
                        </tr>
...
const paymentList= computed(() => store.state.Administrator.payments.data);
...
function getPaymentRequests() {
    store.dispatch('Administrator/getPaymentRequests');
}
...
function approvePayment(id) {
  store.dispatch('Administrator/approvePayment', id)
    .then(() => {
      getPaymentRequests(); // Refresh list after approval
    })
    .catch(error => {
