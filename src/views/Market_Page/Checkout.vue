    ...
    <div class="flex flex-col min-h-screen">
      <div class="p-5 flex-grow">
        ...
        <div v-if="checkoutList.length"
          class="flex flex-wrap items-center gap-2 border-2 border-[#608C54] py-5 p-2 rounded-md shadow-md mt-4">
        ...
        <!-- Main Order Section -->
        <div
          class="mt-6 w-full mx-auto p-3 bg-white shadow-md border-2 border-[#608C54] rounded-lg">
          <!-- Header -->
          <div class="flex flex-wrap justify-between items-center mb-4 border-b pb-2">
            ...
            <div
              class="hidden sm:grid grid-cols-3 gap-10 text-gray-600 text-sm text-center font-semibold w-full sm:w-1/2 mt-2 sm:mt-0">
              <p>Unit price</p>
              <p>Quantity</p>
              <p>Item subtotal</p>
            </div>
          ...
          <div class="overflow-y-auto border rounded-md p-2 max-h-[30rem]">
            <div v-for="checkout in checkoutList" :key="checkout.id"
              class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <div class="flex space-x-4">
                <img v-if="checkout?.product_img?.length" :src="checkout.product_img[0]"
                  alt="Product Image" class="w-20 h-20 rounded-md object-cover" />
                <div>
              ...
              <div
                class="grid grid-cols-3 gap-4 text-center w-full sm:w-1/2 text-sm font-medium">
                <p>₱{{ checkout.price }}</p>
          ...
          <div
            class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-6 mt-6">
            ...
            <div class="col-span-1 xl:col-span-2 pt-2">
              ...
              <div class="flex flex-wrap gap-4">
                <button @click="selectedPayment = 'GCash'"
                  :class="[ 'px-4 py-2 border rounded-md focus:outline-none transition', selectedPayment === 'GCash' ? 'bg-[#608C54] text-white' : 'bg-white text-black' ]">
                  E-Wallet
                </button>
                <button @click="selectedPayment = 'COD'"
                  :class="[ 'px-4 py-2 border rounded-md focus:outline-none transition', selectedPayment === 'COD' ? 'bg-[#608C54] text-white' : 'bg-white text-black' ]">
                  COD
                </button>
              </div>
            </div>
...
const checkoutList = computed(() => store.state.Consumer.checkout.data);
...
function getCheckoutList() {
    store.dispatch('Consumer/getCheckoutList')
}
...
const shippingSubtotal = computed(() => {
  return checkoutList.value.reduce((total, item) => {
    return total + shippingFeePerItem * item.quantity;
  }, 0);
});
...
const placeOrderSelected = async () => {
  ...
  if (selectedItems.length === 0) {
    // Optional: Show a toast or alert to select at least one item
    return;
  }

  for (const checkout of selectedItems) {
    try {
      await store.dispatch('Consumer/placeOrderSelected', { id: checkout.id });
    } catch (error) {
      console.error('Checkout failed for item:', checkout.id, error);
    }
  }

  router.push('/market'); // Navigate back to market after placing the order
};
